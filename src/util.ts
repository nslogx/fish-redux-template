...
export async function checkTemplatesFolder(context: any) {
  ...
  if (!(await fs.exists(config.pageTemplatesFolderPath))) {
    await copyFolder(
      ...
      config.pageTemplatesFolderPath,
      ''
    );
  }
  ...
  if (!(await fs.exists(config.componentTemplatesFolderPath))) {
    await copyFolder(
      path.join(context.extensionPath, `templates${path.sep}component`),
      config.componentTemplatesFolderPath,
      ''
    );
  }
  ...
  if (!(await fs.exists(config.adapterTemplatesFolderPath))) {
    await copyFolder(
      path.join(context.extensionPath, `templates${path.sep}adapter`),
      config.adapterTemplatesFolderPath,
      ''
    );
  }
